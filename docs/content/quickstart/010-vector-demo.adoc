...
[IMPORTANT]
====
Execute the following command in a _new_ terminal window.  This command is provided by GeoWave's embedded GeoServer, which is not part of the command-line tools JAR described in the link:installation-guide.html#install-from-source[Installation Guide^, window="_blank"].  If it is not available, run a GeoServer with the link:userguide.html#geoserver-plugin[GeoWave GeoServer plugin^, window="_blank"] installed and point the CLI at it with link:commands.html#config-geoserver[`geowave config geoserver`].
====
...
[NOTE]
====
RocksDB only supports a single connection to the database, because of this, you will be unable to perform queries or other data store operations with the CLI while GeoServer maintains a connection to it. If you would like the capability to do both simultaneously, you can use a data store that supports multiple connections, such as one of the link:commands.html#standalone-commands[standalone data stores, window="blank"].
====
