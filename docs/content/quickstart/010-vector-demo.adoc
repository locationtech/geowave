...
NOTE: The RocksDB data store cannot run on Apple Silicon Macs, because the `rocksdbjni` 6.19.3 library GeoWave uses has no `osx-arm64` native library.  On Apple Silicon, use the FileSystem data store instead, which also needs no external services: `geowave store add -t filesystem --gwNamespace geowave.gdelt --dir . gdelt`.
...
Now that we have prepared our vector and KDE data, we can visualize it by using the GeoServer plugin.  The command-line tools can run GeoServer themselves, with GeoWave already available to it.
...
[WARNING]
====
GeoServer ${geoserver.version} has known critical vulnerabilities, for example CVE-2024-36401, which allows unauthenticated remote code execution.  `geowave gs run` is for evaluating GeoWave on your own machine only.  It listens on 127.0.0.1 by default, and it must not be exposed to untrusted networks or used as a deployment.
====

[NOTE]
====
GeoServer ${geoserver.version} predates Java 21.  The WMS steps in this guide work, but WFS GetFeature requests fail with "Unsupported class file major version 65" until GeoWave moves to a newer GeoServer.  That includes the WFS output formats in GeoServer's Layer Preview, and the GetFeature requests that WFS-T clients use to read features.
====

The command-line tools do not include GeoServer's web application, so download the GeoServer ${geoserver.version} web archive (WAR), the version GeoWave is built against, and unpack it into a `geoserver` directory in the working directory:

[source, bash]
----
$ curl -O https://repo.osgeo.org/repository/release/org/geoserver/web/gs-web-app/${geoserver.version}/gs-web-app-${geoserver.version}.war
$ unzip -q gs-web-app-${geoserver.version}.war -d geoserver
----

This only needs to be done once.  GeoServer keeps its configuration, such as the layers and styles added below, in `geoserver/data`.

[IMPORTANT]
====
Execute the following command in a _new_ terminal window, from the working directory.
====

[source, bash]
----
$ geowave gs run -d geoserver
----
...
If port 8080 is already in use, add `-p <port>` to the command and tell the CLI where GeoServer is with link:commands.html#config-geoserver[`geowave config geoserver`], for example `+geowave config geoserver http://localhost:<port>/geoserver+`.  An existing GeoServer with the link:userguide.html#geoserver-plugin[GeoWave GeoServer plugin^, window="_blank"] installed can be used the same way instead of `geowave gs run`.

[NOTE]
====
RocksDB only supports a single connection to the database, because of this, you will be unable to perform queries or other data store operations with the CLI while GeoServer maintains a connection to it. If you would like the capability to do both simultaneously, use the FileSystem data store described at the start of this guide, or a data store that runs as its own service, such as Accumulo, HBase, Cassandra or Redis (see link:userguide.html#adding-data-stores[Adding Data Stores^, window="_blank"] in the User Guide).
====
