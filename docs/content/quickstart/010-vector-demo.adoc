...
[WARNING]
====
`geowave gs run` is for evaluating GeoWave on your own machine only.  GeoServer starts with its default administrator account and a configuration that has not been secured.  The command listens on 127.0.0.1 by default, and it must not be exposed to untrusted networks or used as a deployment.
====
