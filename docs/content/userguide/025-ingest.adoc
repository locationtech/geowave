...
2. Add any number of statistics to those data types by using the link:commands.html#stat-calc[`stat add`] command.
...
[source]
----
$ JAVA_TOOL_OPTIONS="-DSIMPLE_FEATURE_CONFIG_FILE=myconfigfile.json" geowave ingest localtogw ./ingest_data mystore myindex
----

[NOTE]
====
`SIMPLE_FEATURE_CONFIG_FILE` is a Java system property, so it must reach the JVM that runs the CLI.  `JAVA_TOOL_OPTIONS` works regardless of how the `geowave` command is set up.  When running the command-line tools JAR directly, the property can instead be passed before `-jar`, e.g. `java -DSIMPLE_FEATURE_CONFIG_FILE=myconfigfile.json -jar geowave-deploy-<version>-tools.jar ingest localtogw ./ingest_data mystore myindex`.
====
