...
The GeoWave GeoServer plugin JAR can be link:devguide.html#geoserver-plugin[packaged from the GeoWave source^, window="_blank"].  Starting with GeoWave 3.0, release binaries are published on link:https://github.com/locationtech/geowave/releases[GitHub Releases^, window="_blank"].
...
[NOTE]
====
When tested under link:commands.html#geoserver-run[`geowave gs run`^, window="_blank"], which runs GeoServer ${geoserver.version} on Java 21, WFS 1.0.0, 1.1.0 and 2.0.0 GetFeature requests all failed with "Unsupported class file major version 65".  Transaction requests were still processed, but a WFS-T client cannot read features back through GetFeature.  GeoServer ${geoserver.version} predates Java 21, so the same failure is expected wherever the GeoWave GeoServer plugin runs in GeoServer ${geoserver.version} on Java 21.  WMS is not affected.
====
...
NOTE: When its JSON file lists any users, the JSON provider (`jsonFile`) reads the requester from Spring Security's `SecurityContextHolder`, so Spring Security (`spring-security-core`) must then be on the classpath. GeoServer provides it, and the GeoWave tools JAR includes it with GeoServer's libraries, but `geowave-adapter-auth` declares it as a provided dependency and does not bring it along. Without an _Authorization Data URL_ the provider grants no authorizations and does not need Spring Security. That is what the vector data store uses when no _Authorization Management Provider_ is set, since `jsonFile` is the first provider it finds.
