...
- Zeppelin Bootstrap: http://s3.amazonaws.com/geowave/latest/scripts/emr/jupyter/bootstrap-zeppelin.sh
...
The Zeppelin notebook server is launched at cluster creation as a link:http://upstart.ubuntu.com/[Upstart] service. If Zeppelin should stop working or need to be restarted after the cluster has been created, you can do so by following these steps.
