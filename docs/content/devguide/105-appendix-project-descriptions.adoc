...
1.1+| 4.1+| deploy           | GeoWave Deployment Configurations     | Maven profiles that package the command-line tools JAR, the GeoServer plugin, and the Accumulo and HBase server-side JARs (see <<020-packaging.adoc#packaging, Packaging GeoWave Builds>>).

1.1+| 4.1+| dev-resources    | GeoWave Development Resources         | Development resources and settings for GeoWave.

1.1+| 4.1+| docs             | GeoWave Documentation                 | Documentation, primarily in the form of asciidoc that can be compiled into a variety of formats including HTML, PDF, EPUB, and others (the site at link:https://locationtech.github.io/geowave/[https://locationtech.github.io/geowave/, window="_blank"] is built from this project and published to GitHub Pages by the `Publish` workflow).
...
3.1+| 2.1+| accumulo-embed   | GeoWave Embedded Accumulo             | Adds `geowave util accumulo run` to run a local Accumulo server for development and testing.
3.1+| 2.1+| cassandra-embed  | GeoWave Cassandra Embedded Server     | Adds `geowave util cassandra run` to run a local Cassandra server for development and testing.
3.1+| 2.1+| dynamodb-embed   | GeoWave DynamoDB Embedded Server      | Adds `geowave util dynamodb run` to run a local DynamoDB server for development and testing.
3.1+| 2.1+| geoserver-embed  | GeoWave Embedded GeoServer            | Adds `geowave gs run` to run a local GeoServer for development and testing.
3.1+| 2.1+| hbase-embed      | GeoWave Embedded HBase                | Adds `geowave util hbase run` to run a local HBase server for development and testing.
3.1+| 2.1+| redis-embed      | GeoWave Redis Embedded Server         | Adds `geowave util redis run` to run a local Redis server for development and testing.
...
3.1+| 2.1+| filesystem       | GeoWave FileSystem DataStore          | GeoWave data store on a local file system, using Java NIO.
3.1+| 2.1+| hbase            |                                       | GeoWave HBase projects.
4.1+| 1.1+| core             | GeoWave HBase                         | GeoWave data store on Apache HBase.
4.1+| 1.1+| coprocessors     | GeoWave HBase Coprocessors            | HBase coprocessors for GeoWave server-side operations, packaged into the HBase coprocessor JAR.
...
1.1+| 4.1+| migration        | GeoWave Migrations                    | Adds `geowave util migrate`, which migrates data in a GeoWave data store to be compatible with the CLI version.
