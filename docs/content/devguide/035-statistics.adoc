...
New statistics can be implemented by extending the appropriate statistic type ({core-store}/core/store/api/IndexStatistic.java[`IndexStatistic`], {core-store}/core/store/api/DataTypeStatistic.java[`DataTypeStatistic`], or {core-store}/core/store/api/FieldStatistic.java[`FieldStatistic`]) and implementing a corresponding {core-store}/core/store/api/StatisticValue.java[`StatisticValue`].  It is recommended that a public static `STATS_TYPE` variable be made available to make the `StatisticType` of the statistic readily available to users.
