...
. Locate the green “Code” button near the top right of the file navigation section.
...
[start=4]
. Expand the clone and download options by clicking on the green “Code” button.
...
. Navigate to the arguments tab and set values for the "VM arguments" field. Example: `-ea -DtestStoreType=ROCKSDB -DtestStoreOptions=enableServerSideLibrary=false` will run the test suite with RocksDB as the data store. RocksDB is also the default when `testStoreType` is not set, but it cannot run on Apple Silicon; use `-DtestStoreType=FILESYSTEM` there instead. Note: The `enableServerSideLibrary` option only applies to Accumulo and HBase and is true by default.
