...
NOTE: Use `clean` when rebuilding. The HBase and raster modules generate protobuf sources into `src/main/java`, and the HBase step rewrites the `.proto` files in place before compiling them, so a rebuild over a dirty tree can produce doubled class names such as `AggregationProtosClientClient`.

==== Apple Silicon

Three of the protobuf toolchain pins predate arm64 macOS, so parts of the build cannot run natively on Apple Silicon.

The raster and HBase modules pin protoc to 2.5.0 for HBase wire compatibility, and Google never published an `osx-aarch_64` binary for it, so the build stops with `Unsupported platform: protoc-2.5.0-osx-aarch_64.exe`. The `osx-x86_64` binary runs fine under Rosetta. Extract it from the `protoc-jar` artifact already in your local repository and point the build at it:

[source, bash]
----
$ unzip -o -j ~/.m2/repository/com/github/os72/protoc-jar/*/protoc-jar-*.jar \
    'bin/2.5.0/protoc-2.5.0-osx-x86_64.exe' -d /tmp/protoc
$ chmod +x /tmp/protoc/protoc-2.5.0-osx-x86_64.exe
$ mvn clean install -DskipTests -Dspotbugs.skip \
    -DprotocCommand=/tmp/protoc/protoc-2.5.0-osx-x86_64.exe
----

That gets the whole reactor as far as `geowave-grpc-protobuf`, which is the rest of the problem: it needs protoc 3.17.1 and `protoc-gen-grpc-java` 1.38.0, and neither publishes an `osx-aarch_64` binary either. `-DprotocCommand` cannot help, because those protos are proto3 and the 2.5.0 compiler only understands proto2. Until the protobuf and gRPC pins move, `geowave-grpc-protobuf`, `geowave-grpc-server` and `geowave-test` have to be built somewhere else.

All three pins lift together with HBase 3, which uses a shaded protobuf 3.x.
