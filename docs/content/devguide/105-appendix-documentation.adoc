...
The documentation is written in http://www.methods.co.nz/asciidoc/index.html[AsciiDoc^] which is a plain-text markup format that can be created using any text editor and read “as-is”, or rendered to several other formats like HTML, PDF or EPUB.
...
To build all the content used for the entire finished web page or the generated PDF for the link:https://locationtech.github.io/geowave/[GeoWave website, window="_blank"], use the following command. Like the rest of the build, it requires JDK 21.

[source, bash]
----
$ cd geowave
$ ./mvnw -P {FORMAT} -pl docs install -DskipTests -Dspotbugs.skip <1>
----
<1> Supported formats include 'pdf' and 'html' (no quotes).

The documentation will be available for inspection in the `geowave/target/site/` directory. See <<015-building.adoc#build-docs, Building Docs>> for building the Javadocs and the Python bindings documentation.
