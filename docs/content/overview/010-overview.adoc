...
In order to handle a multitude of input data types, an adapter is needed to describe the input data type so that can be translated to a format that GeoWave understands.  Among others, GeoWave provides a data adapter implementation that supports link:https://docs.geotools.org/[GeoTools^, window="_blank"] SimpleFeatures by default, which should cover most use cases for vector data.
