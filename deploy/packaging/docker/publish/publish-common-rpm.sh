...
if command -v aws >/dev/null 2>&1 ; then
	if [[ ! -z "$GEOWAVE_VERSION_URL" ]]; then
		...
		aws s3 cp --acl public-read ${WORKSPACE}/examples/data/notebooks/jupyter/geowave-gdelt.ipynb s3://geowave-notebooks/${GEOWAVE_VERSION_URL}/notebooks/geowave-gdelt.ipynb --quiet
	else
