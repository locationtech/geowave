...
public class TemporalQuery extends BasicQuery {
	...
	private static Constraints createTemporalConstraints(
		...
		for (TemporalRange range : contraints.constraints) {
			constraintsPerDimension.put(TimeDefinition.class, new NumericRange(
					range.getStartTime().getTime(), range.getEndTime()
							.getTime()));
		}

		Constraints constraints = new Constraints(constraintsPerDimension);
