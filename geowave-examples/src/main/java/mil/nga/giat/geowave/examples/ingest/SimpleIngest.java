...
public class SimpleIngest {
	...
	public static void main(String[] args) {
		...
		SimpleIngest si = new SimpleIngest();

		try {
			BasicAccumuloOperations bao = si.getAccumuloInstance(args[0], args[1], args[2], args[3], args[4]);
			si.GenerateGrid(bao);
		} catch (Exception e) {
	...
	/***
	 * DataStore is essentially the controller that take the accumulo information, geowave configuration, and data type, and 
	 * inserts/queries from accumulo
	...
	protected DataStore getGeowaveDataStore(BasicAccumuloOperations instance){
		...
		return new AccumuloDataStore(new AccumuloIndexStore(instance), new AccumuloAdapterStore(instance), instance);
	}
