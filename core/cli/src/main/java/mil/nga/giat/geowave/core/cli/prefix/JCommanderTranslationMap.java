...
public class JCommanderTranslationMap
{
	...
	// classpath.
	/**
	 * Parameter annotation identifier for the names field, denoting
	 * CLI-specified names a field can be called through
	 */
	public static final String NAMES_MEMBER = "names";
	/** Parameter annotation identifier for specifying if a field is required */
	...
	/** Parameter annotation identifier for specifying if a field is a password */
	public static final String PASSWORD_MEMBER = "password";
	/** Separator value to be used */
	 ...
	 * 
	 * @return Unmodifiable collection of objects generated by
	 *         createFacadeObjects()
	 */
	 ...
	 * 
	 * @param newFieldName
	 *            A created facade's field (e.g. field_0)
	 * @param item
	 *            Class object this translation and parameter object applies to
	 * @param param
	 *            Parameter object this translation applies to
	 * @param prefix
	 *            Prefix to apply to the translation
	 * @param member
	 *            Annotation object applied to the class object
	 */
		...
		try {
			...
			for (Map.Entry<String, TranslationEntry> mapEntry : translations.entrySet()) {
				...
				if (!entry.isMethod()) {
					...
					// target METHOD, so we'll only clone annotations that can
					// target FIELD.
					...
					// so we can clone annotations. This method is ugly. Do not
					// look at it.
				...
				// a prefix for this class(in ParseContext), then we apply it to
				// the attributes by iterating over the annotations, looking for
				// a 'names' member variable, and overriding the values one by
				// one.
				...
				// implemented as of 1.55, an unreleased version.
				...
				// doesn't matter, but it's used for translation in transMap.
	 ...
	 * 
	 * @param field
	 *            Field to get the annotation and attributes for
	 * @param names
	 *            New member names to apply
	 */
	...
	/**
	 * Iterate the annotations, look for a boolean parameter, and set it to
	 * false.
	 * 
	 * @param booleanMemberName
	 *            Name of a boolean field to disable
	 * @param field
	 *            Field to look at attributes for
	 */
		...
		for (Annotation annotation : fieldAttributes.getAnnotations()) {
			if (annotation.getTypeName().startsWith(
				...
				if (requiredMember != null) {
					...
					// This is KEY! For some reason, the existing annotation
					// will not be modified unless you call 'setAnnotation'
					// here. I'm guessing 'getAnnotation()' creates a copy.
					fieldAttributes.setAnnotation(annotation);
