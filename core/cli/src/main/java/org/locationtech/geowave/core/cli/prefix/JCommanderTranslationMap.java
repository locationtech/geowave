...
public class JCommanderTranslationMap {
  ...
  /** Java 8 reports "1.8"; every later release reports "9", "11", "17" and so on. */
  private static final boolean SUPPORTS_LOOKUP_DEFINE =
      !System.getProperty("java.specification.version", "1.8").startsWith("1.");
  ...
  public void createFacadeObjects() {
    ...
    try {
      ...
      for (final CtClass clz : createdClasses.values()) {
        final Class<?> toClass = defineClass(clz);
  ...
  /**
   * Java 8 and Java 9+ need different Javassist entry points, and each throws on the other.
   *
   ...
   */
  private static Class<?> defineClass(final CtClass clz) throws CannotCompileException {
    if (SUPPORTS_LOOKUP_DEFINE) {
      return clz.toClass(GeneratedClassAnchor.class);
    }
    return clz.toClass();
  }
