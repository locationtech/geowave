 ...
 *
 * <p>Although there are no approximation guarantees, it appears to work well with adequate data and
 * a large number of histogram bins.
 */
      ...
      if ((csum / totalCount) >= q) {
        ...
        final double r =
            bins.get(b - 1).lowerBound
                + ((((q * totalCount) - csum)
                        * (bins.get(b).lowerBound - bins.get(b - 1).lowerBound))
                    / (bins.get(b).count));
        return r;
      }
    }
