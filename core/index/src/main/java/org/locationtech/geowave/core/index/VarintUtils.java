...
/**
 * Based on {@link com.clearspring.analytics.util.Varint}. Provides additional
 * functionality to encode varints directly to ByteBuffers.
 */
public class VarintUtils
{
	...
	/**
	 * Convert an int to an int that uses zig-zag encoding to prevent negative
	 * numbers from using the maximum number of bytes.
	 * 
	 * @see {@code com.clearsprint.analytics.util.Varint}
	 */
	@VisibleForTesting
	static int signedToUnsignedInt(
			int value ) {
	 ...
	 */
	@VisibleForTesting
	static int unsignedToSignedInt(
			int value ) {
		int temp = (((value << 31) >> 31) ^ value) >> 1;
		return temp ^ (value & (1 << 31));
	}

	public static int signedIntByteLength(
			int value ) {
		return unsignedIntByteLength(signedToUnsignedInt(value));
	}

	public static int unsignedIntByteLength(
			int value ) {
	...
	public static void writeSignedInt(
			...
			ByteBuffer buffer ) {
		writeUnsignedInt(
				signedToUnsignedInt(value),
				buffer);
	}

	public static void writeUnsignedInt(
			int value,
			ByteBuffer buffer ) {
	...
	/**
	 * Convert a long to a long that uses zig-zag encoding to prevent negative
	 * numbers from using the maximum number of bytes.
	 * 
	 * @see {@code com.clearsprint.analytics.util.Varint}
	 */
	@VisibleForTesting
	static long signedToUnsignedLong(
			long value ) {
	 ...
	 */
	@VisibleForTesting
	static long unsignedToSignedLong(
			long value ) {
		long temp = (((value << 63) >> 63) ^ value) >> 1;
		return temp ^ (value & (1L << 63));
	}

	public static int signedLongByteLength(
			long value ) {
		return unsignedLongByteLength(signedToUnsignedLong(value));
	}

	public static int unsignedLongByteLength(
			long value ) {
	...
	public static void writeSignedLong(
			long value,
			ByteBuffer buffer ) {
		writeUnsignedLong(
				signedToUnsignedLong(value),
				buffer);
	}

	public static void writeUnsignedLong(
			long value,
			ByteBuffer buffer ) {
	...
	public static long readSignedLong(
			ByteBuffer buffer ) {
		return unsignedToSignedLong(readUnsignedLong(buffer));
	}

	public static long readUnsignedLong(
			ByteBuffer buffer ) {
		long value = 0;
		int i = 0;
		long currByte;
	 ...
	 */
	public static int timeByteLength(
			long time ) {
	 ...
	 *            the timestamp
	 * @param buffer
	 *            the {@code ByteBuffer} to write the timestamp to
	 */
	...
	/**
	 * Read a timestamp from a {@code ByteBuffer} that was previously encoded
	 * with {@link #writeTime}.
	 * 
	 * @param buffer
