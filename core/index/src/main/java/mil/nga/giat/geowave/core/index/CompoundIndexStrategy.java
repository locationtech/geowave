...
public class CompoundIndexStrategy implements NumericIndexStrategy {
	...
	public CompoundIndexStrategy(final NumericIndexStrategy subStrategy1, final NumericIndexStrategy subStrategy2) {
		this.subStrategy1 = subStrategy1;
		this.subStrategy2 = subStrategy2;
		init();
		defaultNumberOfRanges = (int) Math.ceil(Math.pow(2, getNumberOfDimensions()));
	}
	...
	public NumericIndexStrategy[] getSubStrategies() {
		return new NumericIndexStrategy[] { subStrategy1, subStrategy2 };
	}
	...
	public void fromBinary(final byte[] bytes) {
		...
		subStrategy1 = PersistenceUtils.fromBinary(delegateBinary1, NumericIndexStrategy.class);
		subStrategy2 = PersistenceUtils.fromBinary(delegateBinary2, NumericIndexStrategy.class);
		init();
		defaultNumberOfRanges = (int) Math.ceil(Math.pow(2, getNumberOfDimensions()));
	}
	...
	public int[] getNumberOfDimensionsPerIndexStrategy() {
		return new int[] { subStrategy1.getOrderedDimensionDefinitions().length,
	 ...
	 */
	public ByteArrayId composeByteArrayId(final ByteArrayId id1, final ByteArrayId id2) {
	 ...
	 */
	public ByteArrayId[] decomposeByteArrayId(final ByteArrayId id) {
		...
		buf.get(bytes1);
		buf.get(bytes2);
		return new ByteArrayId[] { new ByteArrayId(bytes1), new ByteArrayId(bytes2) };
	}

	private List<ByteArrayId> composeByteArrayIds(final List<ByteArrayId> ids1, final List<ByteArrayId> ids2) {
		final List<ByteArrayId> ids = new ArrayList<>(ids1.size() * ids2.size());
		for (final ByteArrayId id1 : ids1) {
			for (final ByteArrayId id2 : ids2) {
				ids.add(composeByteArrayId(id1, id2));
			}
		}
		return ids;
	}

	private ByteArrayRange composeByteArrayRange(final ByteArrayRange rangeOfStrategy1,
			final ByteArrayRange rangeOfStrategy2) {
		final ByteArrayId start = composeByteArrayId(rangeOfStrategy1.getStart(), rangeOfStrategy2.getStart());
		final ByteArrayId end = composeByteArrayId(rangeOfStrategy1.getEnd(), rangeOfStrategy2.getEnd());
		return new ByteArrayRange(start, end);
	}

	private List<ByteArrayRange> getByteArrayRanges(final List<ByteArrayRange> ranges1,
			final List<ByteArrayRange> ranges2) {
		final List<ByteArrayRange> ranges = new ArrayList<>(ranges1.size() * ranges2.size());
		for (final ByteArrayRange range1 : ranges1) {
			for (final ByteArrayRange range2 : ranges2) {
				final ByteArrayRange range = composeByteArrayRange(range1, range2);
				ranges.add(range);
			}
		}
		return ranges;
	...
	@Override
	public List<ByteArrayRange> getQueryRanges(final MultiDimensionalNumericData indexedRange, final IndexMetaData... hints) {
		return getQueryRanges(indexedRange, -1, hints);
	}

	@Override
	public List<ByteArrayRange> getQueryRanges(final MultiDimensionalNumericData indexedRange,
		...
		if (maxEstimatedRangeDecomposition < 1) {
			rangeForStrategy1 = subStrategy1.getQueryRanges(ranges[0], extractHints(hints, 0));
			rangeForStrategy2 = subStrategy2.getQueryRanges(ranges[1], extractHints(hints, 1));
		} else {
			final int maxEstRangeDecompositionPerStrategy = (int) Math.ceil(Math.sqrt(maxEstimatedRangeDecomposition));
			rangeForStrategy1 = subStrategy1.getQueryRanges(ranges[0], maxEstRangeDecompositionPerStrategy,
					extractHints(hints, 1));
			final int maxEstRangeDecompositionStrategy2 = maxEstimatedRangeDecomposition / rangeForStrategy1.size();
			rangeForStrategy2 = subStrategy2.getQueryRanges(ranges[1], maxEstRangeDecompositionStrategy2,
					extractHints(hints, 2));
		}
		final List<ByteArrayRange> range = getByteArrayRanges(rangeForStrategy1, rangeForStrategy2);
		return range;
	}

	@Override
	public List<ByteArrayId> getInsertionIds(final MultiDimensionalNumericData indexedData) {
		return getInsertionIds(indexedData, defaultNumberOfRanges);
	}
	...
	public List<ByteArrayId> getInsertionIds(final MultiDimensionalNumericData indexedData,
		...
		final List<ByteArrayId> rangeForStrategy1 = subStrategy1.getInsertionIds(ranges[0],
				maxEstDuplicatesPerStrategy);
		final int maxEstDuplicatesStrategy2 = maxEstimatedDuplicateIds / rangeForStrategy1.size();
		final List<ByteArrayId> rangeForStrategy2 = subStrategy2.getInsertionIds(ranges[1], maxEstDuplicatesStrategy2);
		final List<ByteArrayId> range = composeByteArrayIds(rangeForStrategy1, rangeForStrategy2);
		return range;
	}

	private MultiDimensionalNumericData[] getRangesForId(final ByteArrayId insertionId) {
		final ByteArrayId[] insertionIds = decomposeByteArrayId(insertionId);
		return new MultiDimensionalNumericData[] { subStrategy1.getRangeForId(insertionIds[0]),
				subStrategy2.getRangeForId(insertionIds[1]) };
	}

	private MultiDimensionalNumericData[] getRangesForIndexedRange(final MultiDimensionalNumericData indexedRange) {
		...
		return new MultiDimensionalNumericData[] { new BasicNumericDataset(datasetForStrategy1),
				new BasicNumericDataset(datasetForStrategy2) };
	}

	@Override
	public MultiDimensionalNumericData getRangeForId(final ByteArrayId insertionId) {
	...
	@Override
	public long[] getCoordinatesPerDimension(final ByteArrayId insertionId) {
		final ByteArrayId[] insertionIds = decomposeByteArrayId(insertionId);
	...
	private void init() {
		...
		for (final NumericDimensionDefinition definition : strategy2Definitions) {
			...
			if (pos >= 0) {
				...
				precision[pos] = Math.max(precision[pos], strategy2HighestPrecision[twosDefsPosition]);
			} else {
		...
		highestPrecision = Arrays.copyOfRange(precision, 0, baseDefinitions.length);
	}
	...
	public List<IndexMetaData> createMetaData() {
		...
		for (final IndexMetaData metaData : subStrategy1.createMetaData()) {
			result.add(new CompoundIndexMetaDataWrapper(metaData, 0));
		}
		metaDataSplit = result.size();
		for (final IndexMetaData metaData : subStrategy2.createMetaData()) {
			result.add(new CompoundIndexMetaDataWrapper(metaData, 1));
		}
		return result;
	}
	 ...
	 * @return the ByteArrayId for each sub-strategy
	 */
	public static ByteArrayId extractByteArrayId(final ByteArrayId id, final int index) {
		final ByteBuffer buf = ByteBuffer.wrap(id.getBytes());
		final int id1Length = buf.getInt(id.getBytes().length - 4);
		...
		if (index == 0) {
			final byte[] bytes1 = new byte[id1Length];
			buf.get(bytes1);
			return new ByteArrayId(bytes1);
		}

		final byte[] bytes2 = new byte[id.getBytes().length - id1Length - 4];
		buf.get(bytes2);
		return new ByteArrayId(bytes2);
	 ...
	 *
	 */
	private static class CompoundIndexMetaDataWrapper implements IndexMetaData, Persistable {
		...
		public CompoundIndexMetaDataWrapper(final IndexMetaData metaData, final int index) {
			super();
		...
		public void fromBinary(final byte[] bytes) {
			...
			buf.get(metaBytes);
			metaData = PersistenceUtils.fromBinary(metaBytes, IndexMetaData.class);
			index = buf.getInt();
		}
		...
		@Override
		public void insertionIdsAdded(final List<ByteArrayId> ids) {
			metaData.insertionIdsAdded(Lists.transform(ids, new Function<ByteArrayId, ByteArrayId>() {
				@Override
				public ByteArrayId apply(final ByteArrayId input) {
					return extractByteArrayId(input, index);
				}
			}));
		}

		@Override
		public void insertionIdsRemoved(final List<ByteArrayId> ids) {
			metaData.insertionIdsRemoved(Lists.transform(ids, new Function<ByteArrayId, ByteArrayId>() {
				@Override
				public ByteArrayId apply(final ByteArrayId input) {
					return extractByteArrayId(input, index);
				}
