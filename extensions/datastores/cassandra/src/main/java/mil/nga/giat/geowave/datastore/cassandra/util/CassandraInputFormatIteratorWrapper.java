...
/**
 * This is used internally to translate Cassandra rows into native objects
 * (using the appropriate data adapter). It also performs any client-side
 * filtering. It will peek at the next entry in the Cassandra iterator to always
 * maintain a reference to the next value. It maintains the adapter ID, data ID,
 * and original Cassandra key in the GeoWaveInputKey for use by the
 * GeoWaveInputFormat.
 * 
 * @param <T>
 *            The type for the entry
 */
public class CassandraInputFormatIteratorWrapper<T> implements
		Iterator<Entry<GeoWaveInputKey, T>>
{
	private final BaseDataStore dataStore;
	private final PrimaryIndex index;
	private final Iterator<CassandraRow> scannerIt;
	...
	public CassandraInputFormatIteratorWrapper(
			final BaseDataStore dataStore,
			final AdapterStore adapterStore,
			final PrimaryIndex index,
			final Iterator<CassandraRow> scannerIt,
	...
	private void findNext() {
		while ((nextValue == null) && scannerIt.hasNext()) {
			final CassandraRow row = scannerIt.next();
