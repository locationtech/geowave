...
public class CassandraMetadataReader implements MetadataReader {
                  ...
                  public GeoWaveMetadata apply(final Row result) {
                    return new GeoWaveMetadata(
                        query.hasPrimaryId()
                            ? query.getPrimaryId()
                            : result
                                .get(CassandraMetadataWriter.PRIMARY_ID_KEY, ByteBuffer.class)
                                .array(),
                        useSecondaryId(query)
                            ? query.getSecondaryId()
                            : result
                                .get(CassandraMetadataWriter.SECONDARY_ID_KEY, ByteBuffer.class)
                                .array(),
  ...
  private String[] getSelectedColumns(final MetadataQuery query) {
    ...
    if (useSecondaryId(query)) {
      return new String[] {
        CassandraMetadataWriter.PRIMARY_ID_KEY, CassandraMetadataWriter.VALUE_KEY
      };
    }
    return new String[] {
