...
public class HBaseDistributableFilter extends FilterBase {
  ...
  public boolean init(
      final List<QueryFilter> filterList, final CommonIndexModel model, final String[] visList) {
    ...
    if (!rowCells.isEmpty()) {
      ...
      while (it.hasNext()) {
        ...
        if (rowKey == null) {
          rowKey =
              new GeoWaveKeyImpl(
                  cell.getRowArray(), partitionKeyLength, cell.getRowOffset(), cell.getRowLength());
        }
  ...
  protected FlattenedUnreadData aggregateFieldData(
      final Cell cell, final PersistentDataset<CommonIndexValue> commonData) {
    ...
    final FlattenedDataSet dataSet =
        DataStoreUtils.decomposeFlattenedFields(
            qualBuf, valBuf, null, commonIndexFieldIds.size() - 1);
