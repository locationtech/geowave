...
public class GeoWaveFeatureSource extends ContentFeatureStore {
      ...
      bboxStats =
          stats.get(
              VectorStatisticsQueryBuilder.newBuilder()
                  .factory()
                  .bbox()
                  .fieldName(getFeatureType().getGeometryDescriptor().getLocalName())
                  .build()
                  .getId());
    }
    if (bboxStats != null) {
  ...
  protected FeatureReader<SimpleFeatureType, SimpleFeature> getReaderInternal(final Query query)
    ...
    return new GeoWaveFeatureReader(
        query, state.getGeoWaveTransaction(query.getTypeName()), components);
  }

  @Override
  protected FeatureWriter<SimpleFeatureType, SimpleFeature> getWriterInternal(
      final Query query, final int flags) throws IOException {
    final GeoWaveTransactionState state = getDataStore().getMyTransactionState(transaction, this);
