...
public abstract class Sentinel2ImageryProvider {
  ...
  protected static class JSONFeatureIterator implements Iterator<SimpleFeature> {
    ...
    public JSONFeatureIterator(
        Sentinel2ImageryProvider provider, SimpleFeatureType featureType, Iterator<?> iterator) {
    ...
    public SimpleFeature next() {
      ...
      feature.setAttribute(
          SceneFeatureIterator.PROVIDER_NAME_ATTRIBUTE_NAME, provider.providerName());
      ...
      for (int i = 3, icount = descriptorList.size(); i < icount; i++) {
        ...
        try {
          value =
              binding == Date.class
                  ? DateUtilities.parseISO(value.toString())
  ...
  protected static boolean applyCustomCertsFile(
      HttpsURLConnection connection, final File customCertsFile)
  ...
  protected static long copyLarge(
      final InputStream input, final OutputStream output, final int contentLength)
      throws IOException {
    long count = 0;
    int n = 0;
