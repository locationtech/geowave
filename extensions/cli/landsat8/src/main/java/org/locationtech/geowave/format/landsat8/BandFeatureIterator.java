...
public class BandFeatureIterator implements SimpleFeatureIterator {
        ...
        try {
          ...
          for (final String line : htmlLines) {
            ...
            if (endIndex > 0) {
              ...
              featureBuilder.set(
                  BAND_DOWNLOAD_ATTRIBUTE_NAME, getDownloadImage(entityId, path, row, bandId));
  ...
  protected static String getDownloadIndexHtml(
      final String entityId, final int path, final int row) {
    return getDownloadPath(entityId, path, row) + "/index.html";
  }

  protected static String getDownloadImage(
      final String entityId, final int path, final int row, final String bandId) {
