...
public class SwaggerRestParser<T> {
	/**
	 * Reads Geowave CLI operations and parses class fields for particular annotations
	 * ( @Parameter and @ParametersDelegate from JCommander) 
	 * The parsed data is then used to build up JSON objects that can be written
	 * to file and used by Swagger for API documentation and generation
	 */
	...
	public SwaggerRestParser ( DefaultOperation<T> op ) {
		this.operation = op;
	...
	public JsonObject processField(Field f) {
		...
		if(parameter != null) {
			...
			param_json.addProperty("name", f_name);
			...
			if(swaggerType == "array") {
				...
				if(f.getName() == "parameters") {
					...
					items_json.addProperty("type", "string");
					param_json.add("items", items_json);
				} else {
					param_json.addProperty("type", swaggerType);
					JsonObject items_json = new JsonObject();
					items_json.addProperty("type", JavaToSwaggerType(f.getType().getComponentType()));
					param_json.add("items", items_json);
				}
				...
				// since most enum fields contain a description that specifies the permitted values
				// if a more automatic approach is desired in the future just uncomment.
			...
			if(parameter.required() || f_name == "parameters") {
				...
				param_json.addProperty("required", true);
			} else {
				param_json.addProperty("required", false);
			}
	...
	private JsonObject parseParameters() {
		...
		//get the high level attributes from the annotation for the operation (name and description)
		JsonObject op_json = new JsonObject();
		GeowaveOperation gw_annotation =  this.operation.getClass().getAnnotation(GeowaveOperation.class);
		...
		//iterate over the parameters for this operation and add them to the json object
	...
	private String JavaToSwaggerType(Class<?> type) {
