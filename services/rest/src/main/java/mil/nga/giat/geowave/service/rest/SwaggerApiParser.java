...
public class SwaggerApiParser
{
	...
	public SwaggerApiParser(String apiVersion, String apiTitle, String apiDescription) {
	...
	public void AddRoute(RestRoute route) {
		...
		try {
			...
			parser = new SwaggerOperationParser<>(opClass.newInstance());
	...
	public void SerializeSwaggerJson(String filename) {
		Writer writer = null;
		try {
		...
		try {
			...
			gson.toJson(this.routesJson, writer);
			writer.write("}");
			writer.close();
		}
