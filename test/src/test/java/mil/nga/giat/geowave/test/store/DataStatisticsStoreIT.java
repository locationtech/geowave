...
/**
 * This special class is here to test the HBase implementation of merging statistics..
 * Basically it's made more complicated by the fact that HBase doesn't have a MergingCombiner,
 * so we do this special hack here in DataStatistics to merge them as we pull the data out of
 * the iterator.
 */
...
public class DataStatisticsStoreIT {
	...
	public void testInsert() throws IOException {
		...
		DataStatistics<?> stat = new CountDataStatistics<String>(new ByteArrayId("blah"));
		...
		scan.addFamily(new ByteArrayId("STATS").getBytes());
				
		ResultScanner rs = ops.getScannedResults(scan, "GEOWAVE_METADATA");
	...
	public void testQuery() throws IOException {
		...
		for (int i = 0; i < 3; i++) {
			DataStatisticsStore store = dataStore.createDataStatisticsStore();
			DataStatistics<?> stat = new CountDataStatistics<String>(new ByteArrayId("blah2"));
			stat.entryIngested(null, null);
			stat.entryIngested(null, null);
			stat.entryIngested(null, null);
		...
		CountDataStatistics<String> stat = (CountDataStatistics<String>)store.getDataStatistics(adapterId, statisticsId); 
		
		// Non-cached version
		Assert.assertEquals(9, stat.getCount());

		@SuppressWarnings("unchecked")
		CountDataStatistics<String> stat2 = (CountDataStatistics<String>)store.getDataStatistics(adapterId, statisticsId); 

		// Cached version
		Assert.assertEquals(9, stat2.getCount());

		// This code then makes sure that there are indeed 3 records in the database!
		HBaseRequiredOptions opts = (HBaseRequiredOptions)dataStore.getFactoryOptions();
		BasicHBaseOperations ops = BasicHBaseOperations.createOperations(opts);
	...
	public void testDelete() throws IOException {
		...
		for (int i = 0; i < 3; i++) {
			DataStatisticsStore store = dataStore.createDataStatisticsStore();
			DataStatistics<?> stat = new CountDataStatistics<String>(new ByteArrayId("blah3"));
			stat.entryIngested(null, null);
			stat.entryIngested(null, null);
			stat.entryIngested(null, null);
			statisticsId = stat.getStatisticsId();
		...
		Object ignoreVal = store.getDataStatistics(adapterId, statisticsId);
		Assert.assertNotNull(ignoreVal);

		store.removeStatistics(adapterId, statisticsId);
		
		Object val = store.getDataStatistics(adapterId, statisticsId);
		Assert.assertNull(val);
		
		// This code then makes sure that there are indeed 0 records in the database!
		HBaseRequiredOptions opts = (HBaseRequiredOptions)dataStore.getFactoryOptions();
		BasicHBaseOperations ops = BasicHBaseOperations.createOperations(opts);
		...
		scan.setStartRow(statisticsId.getBytes());
		scan.setStopRow(HBaseUtils.getNextPrefix(statisticsId.getBytes()));
		scan.addColumn(new ByteArrayId("STATS").getBytes(), new ByteArrayId("blah3").getBytes());
	...
	@Test
	public void testCombineCommand() throws IOException {

		// Insert Records 
		ByteArrayId statisticsId = null;
		for (int i = 0; i < 3; i++) {
			DataStatisticsStore store = dataStore.createDataStatisticsStore();
			DataStatistics<?> stat = new CountDataStatistics<String>(new ByteArrayId("blah4"));
			stat.entryIngested(null, null);
			stat.entryIngested(null, null);
			stat.entryIngested(null, null);
			statisticsId = stat.getStatisticsId();
		...
		combineCommand.setParameters(null, "blah4");		
		...
		scan.setStartRow(statisticsId.getBytes());
		scan.setStopRow(HBaseUtils.getNextPrefix(statisticsId.getBytes()));
		scan.addColumn(new ByteArrayId("STATS").getBytes(), new ByteArrayId("blah4").getBytes());
