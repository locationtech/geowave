...
public class MapReduceMemoryOperations extends MemoryDataStoreOperations
  ...
  public <T> RowReader<T> createReader(RecordReaderParams<T> readerParams) {
    ...
    ByteArray partitionKey =
        new ByteArray(
            readerParams.getRowRange().getPartitionKey() == null
                ? new byte[0]
    ...
    ByteArrayRange sortRange =
        new ByteArrayRange(
            new ByteArray(
                readerParams.getRowRange().getStartSortKey() == null
                    ? new byte[0]
                    : readerParams.getRowRange().getStartSortKey()),
            new ByteArray(
                readerParams.getRowRange().getEndSortKey() == null
                    ? new byte[0]
                ...
                new QueryRanges(
                    Collections.singleton(
                        new SinglePartitionQueryRanges(
                            partitionKey, Collections.singleton(sortRange)))),
