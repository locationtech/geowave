...
public class BasicMapReduceIT extends AbstractGeoWaveIT {
    ...
    ZipUtils.unZipFile(
        new File(
            MapReduceTestEnvironment.class
                .getClassLoader()
                .getResource(TEST_DATA_ZIP_RESOURCE_PATH)
                .toURI()),
            ...
            new FilenameFilter() {
              ...
              public boolean accept(final File dir, final String name) {
                return FilenameUtils.isExtension(
                    name, new GpxIngestPlugin().getFileExtensionFilters());
              }
            })) {
  ...
  public void testIngestOsmGpxMultipleIndices() throws Exception {
    ...
    MapReduceTestUtils.testMapReduceIngest(
        dataStorePluginOptions, DimensionalityType.ALL, OSM_GPX_INPUT_DIR);
        ...
        TestUtils.getExpectedResults(
            geowaveStore.query(
                QueryBuilder.newBuilder()
                    .addTypeName(adapters[0].getTypeName())
                    .addTypeName(adapters[1].getTypeName())
                    .build()));
  ...
  private static class TestJobRunner extends GeoWaveDedupeJobRunner {
    ...
    public TestJobRunner(
        final DataStorePluginOptions pluginOptions, final ExpectedResults expectedResults) {
      super(pluginOptions);
