  ...
  private static class RasterIngestTester extends RasterIngestRunner {
    ...
    protected void processParameters(final OperationParams params) throws Exception {
      ...
      coverageNameTemplate =
          new Template(
              "name", new StringReader(ingestOptions.getCoverageName()), new Configuration());
    }
  }
  ...
  public void testMosaic() throws Exception {
    ...
    final RasterIngestTester runner =
        new RasterIngestTester(
            dataStoreOptions, analyzeOptions, downloadOptions, ingestOptions, null);
    runner.runInternal(null);

    final StringBuilder str =
        new StringBuilder(StoreFactoryOptions.GEOWAVE_NAMESPACE_OPTION)
            .append("=")
    ...
    str.append(";")
        .append(GeoWaveStoreFinder.STORE_HINT_KEY)
        .append("=")
    ...
    for (int y = 0; y < img.getHeight(); y++) {
      for (int x = 0; x < img.getWidth(); x++) {
        ...
        swappedRedBlueImg.setRGB(
            x, y, ((rgb & 0xff00ff00) | ((rgb & 0xff0000) >> 16) | ((rgb & 0xff) << 16)));
      }
    }
