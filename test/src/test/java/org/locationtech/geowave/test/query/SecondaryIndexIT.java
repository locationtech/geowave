...
public class SecondaryIndexIT {
    ...
    if (dataStoreOptions.getType().equals("accumulo")) {
      ...
      final Connector connector =
          ConnectorPool.getInstance().getConnector(
              options.getZookeeper(), options.getInstance(), options.getUser(),
              options.getPassword());
  ...
  public void testSecondaryIndicesViaDirectQuery() throws IOException {
    ...
    final InternalDataAdapter<SimpleFeature> internalDataAdapter =
        new InternalDataAdapterWrapper(dataAdapter,
    ...
    for (final SecondaryIndexImpl<SimpleFeature> secondaryIndex : allSecondaryIndices) {
      ...
      try (final CloseableIterator<SimpleFeature> results =
          secondaryDataStore.query(
              secondaryIndex, secondaryIndex.getFieldName(), internalDataAdapter, index, query,
    ...
    dataStore.delete(
        QueryBuilder.newBuilder().addTypeName(dataAdapter.getTypeName()).indexName(index.getName())
            .constraints(deleteQuery).build());
  ...
  public void initTest()
    ...
    // PARTIAL
    configs.add(
        new NumericSecondaryIndexConfiguration(NUMERIC_PARTIAL_FIELD, SecondaryIndexType.PARTIAL,
            PARTIAL_LIST));
    configs.add(
        new TemporalSecondaryIndexConfiguration(TEMPORAL_PARTIAL_FIELD, SecondaryIndexType.PARTIAL,
            PARTIAL_LIST));
    configs.add(
        new TextSecondaryIndexConfiguration(TEXT_PARTIAL_FIELD, SecondaryIndexType.PARTIAL,
            PARTIAL_LIST));
    ...
    additionalConstraints
        .put(NUMERIC_JOIN_FIELD, new NumericGreaterThanConstraint(NUMERIC_JOIN_FIELD, number));
    additionalConstraints
        .put(NUMERIC_FULL_FIELD, new NumericGreaterThanConstraint(NUMERIC_FULL_FIELD, number));
    ...
    additionalConstraints
        .put(TEXT_JOIN_FIELD, new TextQueryConstraint(TEXT_JOIN_FIELD, matchValue, true));
    additionalConstraints
        .put(TEXT_FULL_FIELD, new TextQueryConstraint(TEXT_FULL_FIELD, matchValue, true));
    additionalConstraints
        .put(TEXT_PARTIAL_FIELD, new TextQueryConstraint(TEXT_PARTIAL_FIELD, matchValue, true));

    final Date start = DATE_FORMAT.parse("12-24-2015");
    final Date end = DATE_FORMAT.parse("12-26-2015");
    additionalConstraints
        .put(TEMPORAL_JOIN_FIELD, new TemporalQueryConstraint(TEMPORAL_JOIN_FIELD, start, end));
    additionalConstraints
        .put(TEMPORAL_FULL_FIELD, new TemporalQueryConstraint(TEMPORAL_FULL_FIELD, start, end));
    additionalConstraints.put(
  ...
  private void numericJoinAccumulo(final Connector connector) throws TableNotFoundException {
    ...
    scanner.setRange(
        new Range(new Text(Lexicoders.DOUBLE.toByteArray(0d)),
            new Text(Lexicoders.DOUBLE.toByteArray(20d))));

    scanner.fetchColumnFamily(
        new Text(SecondaryIndexUtils
            .constructColumnFamily(dataAdapter.getTypeName(), NUMERIC_JOIN_FIELD)));
    int numResults = 0;
  ...
  private void numericFullAccumulo(final Connector connector) throws TableNotFoundException {
    ...
    scanner.fetchColumnFamily(
        new Text(SecondaryIndexUtils
            .constructColumnFamily(dataAdapter.getTypeName(), NUMERIC_FULL_FIELD)));
    int numResults = 0;
    for (final Entry<Key, Value> entry : scanner) {
      final String dataId =
  ...
  private void temporalFullAccumulo(final Connector connector)
    ...
    scanner.fetchColumnFamily(
        new Text(SecondaryIndexUtils
            .constructColumnFamily(dataAdapter.getTypeName(), TEMPORAL_FULL_FIELD)));
    int numResults = 0;
    for (final Entry<Key, Value> entry : scanner) {
      numResults += 1;
      final String dataId =
  ...
  private void textPartialAccumulo(final Connector connector) throws TableNotFoundException {
    ...
    scanner.fetchColumnFamily(
        new Text(SecondaryIndexUtils
            .constructColumnFamily(dataAdapter.getTypeName(), TEXT_PARTIAL_FIELD)));
    int numResults = 0;
    for (final Entry<Key, Value> entry : scanner) {
      numResults += 1;
      final byte[] cq = entry.getKey().getColumnQualifierData().getBackingArray();
  ...
  private SimpleFeature buildSimpleFeature(
    ...
    builder
        .set(GEOMETRY_FIELD, GeometryUtils.GEOMETRY_FACTORY.createPoint(new Coordinate(lng, lat)));
