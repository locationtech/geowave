...
public class GeoServerIngestIT extends BaseServiceIT {
    ...
    final Envelope env =
        ds.aggregateStatistics(
            VectorStatisticsQueryBuilder.newBuilder()
                .factory()
                .bbox()
                .fieldName(sft.getGeometryDescriptor().getLocalName())
                .build());
  ...
  private static BufferedImage getWMSSingleTile(
    ...
    builder
        .setScheme("http")
        .setHost("localhost")
        .setPort(ServicesTestEnvironment.JETTY_PORT)
        .setPath(WMS_URL_PREFIX)
        .setParameter("service", "WMS")
        .setParameter("version", WMS_VERSION)
        .setParameter("request", "GetMap")
        .setParameter("layers", layer)
        .setParameter("styles", style == null ? "" : style)
        .setParameter("crs", "EPSG:3857")
