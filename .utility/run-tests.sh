...
if [ "$PYTHON_BUILD" == "true" ]; then
  ...
  if [ "$IT_ONLY" == "true" ]; then
    ...
    mvn -q -B verify -am -pl test -Dtest=SkipUnitTests -Dfindbugs.skip -Dspotbugs.skip --add-opens java.base/java.net=ALL-UNNAMED -DfailIfNoTests=false -P $MAVEN_PROFILES
  else
    echo -e "Running unit tests only w/ verify...\n"
    mvn -q -B verify --add-opens java.base/java.net=ALL-UNNAMED -Dformatter.action=validate -P $MAVEN_PROFILES
  fi
fi
