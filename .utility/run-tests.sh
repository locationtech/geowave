...
if [ "$PYTHON_BUILD" == "true" ]; then
  ...
  if [ "$IT_ONLY" == "true" ]; then
    ...
    mvn --add-opens java.base/java.net=ALL-UNNAMED -q -B verify -am -pl test -Dtest=SkipUnitTests -Dfindbugs.skip -Dspotbugs.skip -DfailIfNoTests=false -P $MAVEN_PROFILES
  else
    echo -e "Running unit tests only w/ verify...\n"
    mvn  --add-opens java.base/java.net=ALL-UNNAMED -q -B verify -Dformatter.action=validate -P $MAVEN_PROFILES
  fi
fi
