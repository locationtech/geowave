...
if [ "$TRAVIS_REPO_SLUG" == "mawhitby/geowave" ] && $BUILD_DOCS && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "master" ]; then  
  ...
  # source the build.properties file to get the project.version 
  ...
  git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/mawhitby/geowave gh-pages > /dev/null
