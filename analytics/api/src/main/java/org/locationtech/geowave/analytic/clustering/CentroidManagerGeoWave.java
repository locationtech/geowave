...
public class CentroidManagerGeoWave<T> implements CentroidManager<T> {
  ...
  public CentroidManagerGeoWave(
    ...
    adapter =
        (GeotoolsFeatureDataAdapter)
            adapterStore.getAdapter(centroidInternalAdapterId).getAdapter();
  }
  ...
  private void init(final JobContext context, final Class<?> scope, final Logger logger)
    ...
    try {
      centroidFactory =
          (AnalyticItemWrapperFactory<T>)
              CentroidParameters.Centroid.WRAPPER_FACTORY_CLASS
                  .getHelper()
                  .getValue(context, scope, CentroidItemWrapperFactory.class);
      ...
      LOGGER.error(
          "Cannot instantiate "
              + GeoWaveConfiguratorBase.enumToConfKey(
                  this.getClass(), CentroidParameters.Centroid.WRAPPER_FACTORY_CLASS));
    ...
    final PersistableStore store =
        (PersistableStore)
            StoreParameters.StoreParam.INPUT_STORE.getHelper().getValue(context, scope, null);
            ...
            adapterStore
                .getAdapter(
                    store
                        ...
                        .getAdapterId(centroidDataTypeId))
                .getAdapter();
  }
  ...
  private ToSimpleFeatureConverter<T> getFeatureConverter(
      final List<AnalyticItemWrapper<T>> items, final Class<? extends Geometry> shapeClass) {
  ...
  private static SimpleFeatureType createFeatureType(
      final SimpleFeatureType featureType, final Class<? extends Geometry> shapeClass) {
    try {
  ...
  private static Geometry convert(
      final Geometry value, final Class<? extends Geometry> shapeClass) {
  ...
  private class SimpleFeatureConverter implements ToSimpleFeatureConverter<T> {
    ...
    public SimpleFeatureConverter(
        final FeatureDataAdapter adapter, final Class<? extends Geometry> shapeClass) {
  ...
  private class NonSimpleFeatureConverter implements ToSimpleFeatureConverter<T> {
    ...
    public NonSimpleFeatureConverter(
        final String[] extraDimensionNames, final Class<? extends Geometry> shapeClass) {
      featureType =
  ...
  public void toShapeFile(final String parentDir, final Class<? extends Geometry> shapeClass)
    ...
    try (final FeatureWriter<SimpleFeatureType, SimpleFeature> writer =
      ...
      for (final AnalyticItemWrapper<T> item : centroids) {
        ...
        for (final AttributeDescriptor attrD :
            newFeature.getFeatureType().getAttributeDescriptors()) {
