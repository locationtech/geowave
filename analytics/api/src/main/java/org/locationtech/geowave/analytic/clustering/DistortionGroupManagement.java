          ...
          (CloseableIterator)
              dataStore.query(
                  QueryBuilder.newBuilder()
                      .addTypeName(DistortionDataAdapter.ADAPTER_TYPE_NAME)
                      .indexName(DISTORTIONS_INDEX.getName())
                      .constraints(new BatchIdQuery(batchId))
                      .build())) {
        while (it.hasNext()) {
  ...
  public static class DistortionDataAdapter implements DataTypeAdapter<DistortionEntry> {
    ...
    public DistortionEntry decode(final IndexedAdapterPersistenceEncoding data, final Index index) {
      return new DistortionEntry(
          data.getDataId(), (Double) data.getAdapterExtendedData().getValue(DISTORTION_FIELD_NAME));
    }

    @Override
    public AdapterPersistenceEncoding encode(
        final DistortionEntry entry, final CommonIndexModel indexModel) {
    ...
    public FieldWriter<DistortionEntry, Object> getWriter(final String fieldId) {
      if (DISTORTION_FIELD_NAME.equals(fieldId)) {
        if (distortionVisibilityHandler != null) {
          return (FieldWriter)
              FieldUtils.getDefaultWriterForClass(Double.class, distortionVisibilityHandler);
        } else {
