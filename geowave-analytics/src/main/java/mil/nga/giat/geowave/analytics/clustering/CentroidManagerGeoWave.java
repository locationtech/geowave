...
import mil.nga.giat.geowave.analytics.tools.DirectBasicAccumuloOperationsBuilder;
		...
		CentroidManager<T>
{
	...
	public CentroidManagerGeoWave(
		...
		this.indexId = runTimeProperties.getProperty(CentroidParameters.Centroid.INDEX_ID);
	...
	protected CloseableIterator<T> getRawCentroids(
		...
		final Expression expB1 = factory.property(ClusterFeatureAttribute.BatchId.attrName());
		...
		if (groupID != null) {
			final Expression exp1 = factory.property(ClusterFeatureAttribute.GroupID.attrName());
			...
			final Expression exp1 = factory.property(ClusterFeatureAttribute.ZoomLevel.attrName());
	...
	public void transferBatch(
		...
		while (it.hasNext()) {
			...
			count++;
			store.ingest(
					(WritableDataAdapter<T>) this.adapterStore.getAdapter(new ByteArrayId(
